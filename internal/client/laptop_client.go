...
type LaptopClient struct {
	service pb.CreateLaptopClient
}

func NewLaptopClient(cc *grpc.ClientConn) *LaptopClient {
	service := pb.NewCreateLaptopClient(cc)
...
func (laptopClient *LaptopClient) CreateLaptop(laptop *pb.Laptop) {
	...
	if err != nil {
		...
		if ok && st.Code() == codes.AlreadyExists {
			...
			log.Fatal("cannot craete laptop:", err)
		}
		return
	}
	log.Printf("create laptop:%s", res.Id)
}
...
func (laptopClient *LaptopClient) SearchLaptop(filter *pb.Filter) {
	...
	if err != nil {
		log.Fatal("cannot search laptop:%v", err)
	}
...
func (laptopClient *LaptopClient) UploadImage(laptopID string, imagePath string) {
	...
	if err != nil {
		log.Fatal("cannot upload image:%v", err)
	}
	...
	err = stream.Send(req)
	if err != nil {
		log.Fatal("cannot send image info:%v", err)
	}
	...
	for {
		...
		if err != nil {
			log.Fatal("cannot read chunk to reader:%v", err)
		}
		...
		err = stream.Send(req)
		if err != nil {
			log.Fatal("cannot send chunk to server:%v", err)
		}
	}
